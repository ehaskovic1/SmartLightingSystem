...
//   ON/OFF -> FAULT (fault_detected) => sending FAULT_REPORT 
//   FAULT -> IDLE (manual_reset)
//
// Note:
// - Fault simulation is disabled by default (prob=0). If needed, add a command-line argument.
...
public:
      ...
      timer_heartbeat_(io_)
  {
    ...
    sls::apply_pqc_client(ssl_ctx_);

    transitions_.insert({"IDLE", "UNREGISTERED"});
    transitions_.insert({"UNREGISTERED", "REGISTERING"});
    transitions_.insert({"REGISTERING", "OFF"});
    transitions_.insert({"REGISTERING", "UNREGISTERED"});
    transitions_.insert({"OFF", "ON"});
    transitions_.insert({"ON", "OFF"});
    transitions_.insert({"ON", "FAULT"});
    transitions_.insert({"OFF", "FAULT"});
...
private:
  ...
  struct OutMsg { sls::MsgType t; std::vector<uint8_t> p; };
  std::deque<OutMsg> out_q_;
  bool writing_{false};

  void queue_send(sls::MsgType t, std::vector<uint8_t> payload){
    if(!sock_) return;
    auto self = shared_from_this();
    asio::post(io_, [self, t, payload=std::move(payload)]() mutable {
      self->out_q_.push_back(OutMsg{t, std::move(payload)});
  ...
  void do_write(){
    ...
    sls::async_write_frame(*sock_, msg.t, msg.p,
      [self](const boost::system::error_code& ec, std::size_t){
        if(ec){
          self->schedule_reconnect();
          return;
        }
        // After each write, if there is a pending fault – send it
        if(self->pending_fault_){
          self->pending_fault_ = false;
          self->queue_send(sls::MsgType::FAULT_REPORT,
                           sls::encode_fault_report(self->pending_fault_msg_));
        }
        self->do_write();
      });
  }
  ...
  void send_register(){
    ...
    queue_send(sls::MsgType::REGISTER_REQ, sls::encode_register_req(r));
  ...
  void schedule_heartbeat(){
    ...
    timer_heartbeat_.async_wait([self](const boost::system::error_code& ec){
      if(ec) return;
      if(self->state_ == LampState::ON || self->state_ == LampState::OFF || self->state_ == LampState::FAULT){
        self->send_status();
      }
  ...
  void send_status(){
    ...
    queue_send(sls::MsgType::STATUS_REPORT, sls::encode_status_report(s));
  }

  // pending fault
  bool pending_fault_{false};
  sls::FaultReport pending_fault_msg_{};

  void request_fault(uint8_t code, const std::string& text){
    sls::FaultReport f;
    f.uri = uri_;
    f.zone_id = zone_;
    f.code = code;
    ...
    if(writing_ || !out_q_.empty()){
      pending_fault_ = true;
      pending_fault_msg_ = f;
    } else {
      queue_send(sls::MsgType::FAULT_REPORT, sls::encode_fault_report(f));
    }
  }
  ...
  void maybe_simulate_fault(){
    ...
    if((int)urand(0,999) < fault_prob_per_mille_){
      if(SwitchToState(LampState::FAULT, "fault_detected/FAULT_REPORT")){
        request_fault(1, "fault_detected");
      }
    }
  }
