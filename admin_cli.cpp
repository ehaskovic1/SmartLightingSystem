  ...
  try{
    ...
    for(uint16_t i=0;i<regions;i++){
      ...
      for(uint16_t z=0; z<zones; z++){
        ...
        uint16_t sensor_total = sls::get_u16(p,e);
        uint16_t sensor_fault = sls::get_u16(p,e);

        std::cout<<"  zone "<<zid
                 <<" sum_power_mW="<<pwr
                 <<" | lamps(total="<<lamp_total<<", on="<<lamp_on<<", fault="<<lamp_fault<<")"
                 <<" | sensors(total="<<sensor_total<<", fault="<<sensor_fault<<")"
                 <<"\n";
      }
    }
  } catch(const std::exception& ex){
...
int main(int argc, char** argv){
  ...
  asio::ssl::context ctx(asio::ssl::context::tls_client);
  ctx.set_verify_mode(asio::ssl::verify_none);
  sls::apply_pqc_client(ctx); //dodala 18.feb
